...
def make_x1d_ext0(now,thisfile):
    hdu=fits.PrimaryHDU()
...
def make_x1d_ext1(now,thisfile):
    val1=np.array(['ANY'])
...
def make_x1d_ext2(cdp_dir,outplot):
...
def make_apcorr_ext0(now,thisfile):
    hdu=fits.PrimaryHDU()
...
def make_apcorr_ext1(cdp_dir,outplot):
    print('Figuring out wavelength ranges')
    wmin1A,_=mc.waveminmax('1A')
    _,wmax4C=mc.waveminmax('4C')
